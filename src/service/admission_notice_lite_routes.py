...
def _has_unsupported_control_input(
    request: Request, *, require_explicit_empty_length: bool = False
) -> bool:
    ...
    if content_length is None:
        return require_explicit_empty_length
    try:
...
def register_admission_notice_lite_routes(
    *,
    ...
    """Register L1 routes only when explicitly enabled."""
    ...
    async def create_recognition(request: Request, session_id: str) -> Response:
        if _has_unsupported_control_input(request, require_explicit_empty_length=True):
            return _error_response(
                AdmissionNoticeLiteError(RecognitionErrorReasonLiteV1.INVALID_REQUEST)
            )
        try:
            job = await service.create_recognition(session_id)
